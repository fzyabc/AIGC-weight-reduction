...
│  文档处理器      │ │ 报告解析增强层              │ │    降重引擎（核心）    │
│                 │ │                             │ │                      │
│ · read_docx()   │ │ · parse_pdf()               │ │ ┌──────────────────┐ │
│ · replace_para()│ │ · match_paragraphs()        │ │ │  Transformer     │ │
│ · save_docx()   │ │ · match_paragraphs_enhanced()│ │ │  (规则引擎)       │ │
│ · analyze_doc() │ │ · sliding-window matching    │ │ │                  │ │
│                 │ │ · keyword fingerprint        │ │ │ · 序列词替换      │ │
│ 依赖:           │ │ · low-confidence tag         │ │ │ · 连接词替换      │ │
│  python-docx    │ │ 依赖: PyPDF2                │ │ │ · 打破对称        │ │
└─────────────────┘ └─────────────────────────────┘ │ │ · 注入人味        │ │
                                        │ │ · 句式重构        │ │
                                        │ └──────────────────┘ │
                                        │                      │
...
用户配置 API URL + Key + 模型
  ...
  AITransformer.batch_transform()
       │
       │  对每段调用 LLM API:
       │    system: 固定策略 + 动态Prompt(随机2条) + custom prompt
       │    user:   风险等级提示 + 原文
       │
       │  调用方式: urllib → POST /chat/completions
       │  超时/鉴权/网络异常: 自动触发规则引擎兜底
       │
       ▼
  Fallback Chain:
       AITransformer 失败
           └─> Transformer(level=3) 规则降重兜底
                   └─> 返回“已自动转为规则降重”
       │
       ▼
  写回文档 → 输出
...
用户输入术语保护库
       │
       ▼
Transformer.protect_keywords()
       │
       │  将指定术语替换为 [KEYWORD_0] / [KEYWORD_1] / ...
       │  目的：避免规则引擎或 LLM 改写时误伤术语
       │
       ▼
规则引擎 / AITransformer 执行降重
       │
       ▼
Transformer.restore_keywords()
       │
       │  将占位符恢复为原始术语
       │  不直接改动 docx run 结构，仅作用于段落文本
       │
       ▼
写回文档
```
...
| `Transformer.protect_keywords()` | 将术语替换为占位符，保护专业词不被误改 |
| `Transformer.restore_keywords()` | 在降重完成后恢复术语占位符 |
...
- `/api/reduce` 支持术语保护、混合降重、处理方式回传（ai / rule / none）
...
### report_parser.py — 报告解析增强层

- 使用 `PyPDF2` 提取 PDF 文本
- 正则匹配 AIGC 概率、风险字数
- `match_paragraphs_enhanced()` 三阶段匹配：
  1. 单段模糊匹配
  2. 滑动窗口匹配（相邻 2-3 段合并）
  3. 关键词指纹兜底匹配
- 结果支持 `is_low_confidence` 标记，前端可显示“低置信度匹配”提示
