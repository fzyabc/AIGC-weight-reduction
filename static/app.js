...
async function runReduce() {
  ...
  startProgress(
    selected.length,
    hybridMode ? "混合降重进行中" : "规则降重进行中",
  );
  startLiveProgress(
    selected.length,
    hybridMode ? "实时混合降重监控" : "实时规则降重监控",
  );
  ...
  try {
    ...
    setStatus(
      "active",
      `${modeText}完成! 修改了 ${data.modified_count} 个段落`,
    );
  } catch (err) {
    ...
    if (previewEl.textContent === "点击加载预览...") {
      ...
      try {
        ...
        rulesEl.innerHTML = data.rules_applied
          .map((r) => `<span class="rule-tag">${r}</span>`)
          .join("");
      } catch {
...
function appendParagraphToView(detail) {
  ...
  const existing = list.querySelector(
    `.change-item[data-index="${detail.index}"]`,
  );
  ...
  const strategyTags =
    (Array.isArray(detail.rules) ? detail.rules : [])
  ...
  editable.addEventListener("blur", () => {
    ...
    editable.innerHTML = renderDiffHtml(
      item.dataset.original || detail.original || "",
      newText,
    );
  ...
  item
    ...
    .addEventListener("click", () => retrySingleSegment(item, detail));
  item
    .querySelector(".undo-btn")
    .addEventListener("click", () => undoSingleSegment(item, detail.index));
}
...
function renderReduceResult(data) {
  ...
  const hasFallback =
    ...
    data.details.some(
      (d) =>
        ...
        d.rules.some((rule) =>
          String(rule).includes("AI失败后自动切换规则降重"),
        ),
    );
  ...
  const list = $("#changeList");
  const hasLiveItems = list.querySelectorAll(".change-item").length > 0;

  if (hasLiveItems) {
    // Live preview already rendered paragraphs — merge final data into
    // existing DOM instead of wiping it. Update finalResults with
    // authoritative server data and refresh diff highlights in-place.
    state.finalResults = {};
    data.details.forEach((d) => {
      state.finalResults[String(d.index)] = [d.transformed];
      const existing = list.querySelector(
        `.change-item[data-index="${d.index}"]`,
      );
      if (existing) {
        const editable = existing.querySelector(".change-after");
        if (editable) {
          editable.innerHTML = renderDiffHtml(d.original, d.transformed);
        }
        const badge = existing.querySelector(".method-badge");
        if (badge) {
          const b = getMethodBadge(d.method);
          badge.className = `method-badge ${b.className}`;
          badge.textContent = b.text;
        }
        existing.dataset.method = d.method || "none";
      } else {
        appendParagraphToView(d);
      }
    });
  } else {
    // No live items — full render (e.g. called from continueReduce path)
    state.finalResults = {};
    data.details.forEach((d) => {
      state.finalResults[String(d.index)] = [d.transformed];
    });
...
async function retrySingleSegment(item, detail) {
  ...
  try {
    ...
    const usedFallback =
      data.fallback === "rule" ||
      (Array.isArray(data.rules_applied) &&
        data.rules_applied.some((r) => r.includes("AI失败后自动切换规则降重")));
    if (usedFallback) {
      const fallbackNotice = $("#fallbackNotice");
      const fallbackNoticeText = $("#fallbackNoticeText");
    ...
    if (meta) {
      const tags = (Array.isArray(data.rules_applied) ? data.rules_applied : [])
        ...
        .join("");
      meta.innerHTML =
        tags + (data.ai_error ? buildAiErrorTags(data.ai_error) : "");
    }
...
function buildAiErrorTags(error) {
  ...
  if (error.code) {
    tags.push(
      `<span class="strategy-tag strategy-tag-error">AI错误: ${escapeHtml(String(error.code))}</span>`,
    );
  }
  if (error.status !== undefined && error.status !== null) {
    tags.push(
      `<span class="strategy-tag strategy-tag-error">状态码: ${escapeHtml(String(error.status))}</span>`,
    );
  }
  if (error.attempts) {
    tags.push(
      `<span class="strategy-tag strategy-tag-error">重试次数: ${escapeHtml(String(error.attempts))}</span>`,
    );
  }
  if (error.retry_after) {
    tags.push(
      `<span class="strategy-tag strategy-tag-error">Retry-After: ${escapeHtml(String(error.retry_after))}</span>`,
    );
  }
  if (error.message) {
    tags.push(
      `<span class="strategy-tag strategy-tag-error">${escapeHtml(String(error.message).slice(0, 120))}</span>`,
    );
  }
  return tags.join("");
}
...
function renderDiffHtml(original, transformed) {
  ...
  return newTokens
    .map((token, idx) => {
      ...
      const nearbyIdx = findNearbyTokenIndex(
        origIndexMap,
        token,
        idx,
        usedOrigIndices,
...
function saveAIConfig() {
  ...
  try {
    ...
    localStorage.setItem(
      "aigc_protected_words",
      $("#protectedWords").value || "",
    );
    localStorage.setItem(
      "aigc_hybrid_mode",
      $("#hybridMode").checked ? "1" : "0",
    );
  } catch {
    setStatus("", "本地配置保存失败");
  }
...
async function runAIReduce() {
  ...
  console.log("[AIGC Reducer] runAIReduce live mode start", {
    selected: selected.length,
    build: window.__AIGC_FRONTEND_BUILD__,
  });
